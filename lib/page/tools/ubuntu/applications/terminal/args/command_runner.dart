...
class CommandRunner<T> {
  ...
  Future<Object?> run(Iterable<String> args, Terminal terminal) =>
      Future.sync(() => runCommand(parse(args), terminal));
  ...
  Future<Object?> runCommand(ArgResults topLevelResults, Terminal terminal) async {
    ...
    while (commands.isNotEmpty) {
      if (argResults.command == null) {
        if (argResults.rest.isEmpty) {
          ...
          var similarCommands =
              similarCommandsText(requested, commands.values);
    ...
    return (command.run(terminal)) as T?;
  }

  // Returns help text for commands similar to `name`, in sorted order.
  String similarCommandsText(String name, Iterable<Command<T>> commands) {
  ...
  String similarCommandsTextHelper(String name, Iterable<Command<T>> commands) {
    if (suggestionDistanceLimit <= 0) return '';
    var distances = <Command<T>, int>{};
    var candidates =
    SplayTreeSet<Command<T>>((a, b) => distances[a]! - distances[b]!);
