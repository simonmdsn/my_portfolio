...
class TypeTrainer extends ConsumerStatefulWidget {
  ...
  TypeTrainer({required this.windowKey,
    Key? key,
  }) : super(key: UniqueKey());
...
class _TypeTrainerState extends ConsumerState<TypeTrainer> {
  ...
  Future<void> fetchWords() async {
    ...
    var list = List.generate(1, (index) => split[random.nextInt(split.length)]);
    _text = list.join(' ');
  ...
  Widget build(BuildContext context) {
    final draggableWindow = ref
        .watch(windowManagerProvider.notifier)
        .get(widget.key!) as DraggableWindow;
    ...
    return DraggableWindow(
      ...
      child: ApplicationHolder(
                        ...
                        onChanged: (str) {
                          if (_stopWatch?.isActive ?? true) {
                            _stopWatch = Timer.periodic(
                                ...
                                (timer) => _elapsedMilliseconds++);
                          }
                          if (str.length <=
                              _textReadOnlyController.text.length) {
        ...
        header: Stack(
          children: [
            const Align(
                alignment: Alignment.center, child: Text('Type Trainer')),
            Row(
